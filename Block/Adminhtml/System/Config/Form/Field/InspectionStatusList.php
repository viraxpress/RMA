...
/**
 * Class InspectionStatusList
 *
 * Renders the HTML for the Inspection Status dropdown field in the system configuration.
 */
class InspectionStatusList extends Field
{
    /**
    ...
    {
        $defaultValues = ['Pending', 'In Process', 'Accepted','Rejected'];
        if (!$element->getValue()) {
            $element->setValue(implode(',', $defaultValues));
        }
        $this->setElement($element);
        $this->setData('list_class', 'inspection-status-list');
