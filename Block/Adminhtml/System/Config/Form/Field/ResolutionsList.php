 ...
 *
 * Renders the list of available RMA resolutions in the system configuration.
 */
class ResolutionsList extends Field
{
    /**
     * Generate the HTML for the RMA resolutions list field.
     *
    ...
    protected function _getElementHtml(AbstractElement $element)
    {
        ...
        $readonlyStatuses = ['Refund', 'Replacement'];
        if (empty($rawValues)) {
            $rawValues = $readonlyStatuses;
        }
        $html = '<div id="' . $elementId . '_wrapper">';
        $html .= '<ul class="resolution-list" id="' . $elementId . '_list">';
...
.resolution-list li input[type="text"],.list-add-input input[type="text"] {
    flex: 1;
    padding: 6px;
    margin-right: 8px;
...
.resolution-list li .remove-field,.list-add-input .add-field {
    padding: 6px 10px;
    cursor: pointer !important;
    width: 50px;
}

.resolution-list li .remove-field{
