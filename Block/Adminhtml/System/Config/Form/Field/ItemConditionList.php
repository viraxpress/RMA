...
/**
 * Class ItemConditionList
 *
 * Renders the HTML for the Item Condition field in system configuration.
 */
class ItemConditionList extends Field
{
    /**
    ...
    {
        $defaultValues = ['Unopened', 'Opened but unused', 'Used'];
        if (!$element->getValue()) {
            $element->setValue(implode(',', $defaultValues));
        }
        $this->setElement($element);
        $this->setData('list_class', 'item-condition-list');
